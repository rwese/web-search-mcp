...
import { createDebugLogger, resolveDebug, type DebugLogger } from "../infra/debug.js";
import { normalizeResult, searchRaw, type RawSearchResult } from "../infra/searxng.js";
import { createSessionId, writeSession } from "./session.js";

/**
 * Perform a web search and persist it as a session.
 *
 * Config comes from the environment + XDG config file (see infra/config.ts),
 * with per-call overrides via `options`. Every search writes a session
 * directory under the configured store root, keyed by the returned sessionId.
 */
